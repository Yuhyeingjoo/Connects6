...
void write(char* data, char * archive, char* path ){
	FILE *wp,* cp;
	char* ul_path = "header.bin", *content = "con.bin";
...
{	
	...
	if(sig==0){ 
		...
		w_size = ftell(h);
		rewind(h);
		buff = (char*)malloc(w_size);
		fread(buff,1,w_size,h);
		
		fwrite(num_f,4,1,p);
		fwrite(buff,1,w_size,p);
		
		fseek(c,0,SEEK_END);
		w_size = ftell(c);
		rewind(c);
		buff = (char*)malloc(w_size);
		fread(buff,1,w_size,c);
